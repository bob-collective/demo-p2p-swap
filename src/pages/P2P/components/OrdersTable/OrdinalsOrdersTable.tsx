...
const OrdinalCell = () => (
  <Flex alignItems='center' gap='spacing2'>
    <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <g clipPath='url(#clip0_1521_1620)'>
        <path
...
enum OrdinalOrdersTableColumns {
  ASSET = 'asset',
  PRICE = 'price',
...
type OrdinalOrdersTableRow = {
  id: string;
  [OrdinalOrdersTableColumns.ASSET]: ReactNode;
  [OrdinalOrdersTableColumns.PRICE]: ReactNode;
  [OrdinalOrdersTableColumns.INSCRIPTION_ID]: ReactNode;
  [OrdinalOrdersTableColumns.ACTION]: ReactNode;
};
...
const OrdinalOrdersTable = ({ orders, refetchOrders, ...props }: OrdinalOrdersTableProps): JSX.Element => {
  ...
  const handleOpenFillOrderModal = (order: OrdinalOrder) => setOrderModal({ isOpen: true, type: 'fill', order });

  const handleOpenCancelOrderModal = (order: OrdinalOrder) => setOrderModal({ isOpen: true, type: 'cancel', order });

  const handleCloseAnyOrderModal = () => setOrderModal((s) => ({ ...s, isOpen: false }));

  const columns = [
    { name: 'Asset', id: OrdinalOrdersTableColumns.ASSET },
    { name: 'Price', id: OrdinalOrdersTableColumns.PRICE },
    { name: 'Inscription ID', id: OrdinalOrdersTableColumns.INSCRIPTION_ID },
    { name: '', id: OrdinalOrdersTableColumns.ACTION }
  ];
...
export { OrdinalOrdersTable };
export type { OrdinalOrdersTableProps };
